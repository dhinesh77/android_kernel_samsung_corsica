...
typedef struct
{
    ...
    short (*pDiamondVoice_Config)(int , short *, short *, short *);
...
#define	VOICE_FRAME	160

#ifdef ENABLE_DIAMOND_SOLUTION    
extern void DiamondVoice_Exe(void);
extern short DiamondVoice_Config(int , Int16 *, Int16 *, Int16 *);
#endif
#ifdef ENABLE_NXPEX_SOLUTION  
...
static void work_output_thread(struct work_struct *work)
{
    ...
    for ( i=0 ; i < index ;i++) {
// Tx
        sgDumpData.fp_tx_in->f_op->write( sgDumpData.fp_tx_in, (const char __user *)tx_in_array[i], sizeof(Int16) * VOICE_FRAME, &sgDumpData.fp_tx_in->f_pos );
        sgDumpData.fp_tx_out->f_op->write( sgDumpData.fp_tx_out, (const char __user *)tx_out_array[i], sizeof(Int16) * VOICE_FRAME, &sgDumpData.fp_tx_out->f_pos );
// Rx
        sgDumpData.fp_rx_out->f_op->write( sgDumpData.fp_rx_out, (const char __user *)rx_out_array[i], sizeof(Int16) * VOICE_FRAME, &sgDumpData.fp_rx_out->f_pos );
    }
...
//    printk("\n===============  work_output_thread  =============== \n");
    ...
    sgDumpData.fp_in->f_op->write( sgDumpData.fp_in, (const char __user *)sgCallbackData.pDldata, sizeof(Int16) * VOICE_FRAME, &sgDumpData.fp_in->f_pos );
    sgDumpData.fp_out->f_op->write( sgDumpData.fp_out, (const char __user *)sgCallbackData.pDlOutput_buf, sizeof(Int16) * VOICE_FRAME, &sgDumpData.fp_out->f_pos );
}
	...
	UInt8 isCall16K)
{
    ...
    if (sgInjectData.flag ==1 && index<=MAX_INJECT )
    {
        ...
        {
            memcpy(dlData, inject_array[index], sizeof(Int16) * VOICE_FRAME);
            memcpy(tx_in_array[index], ulData, sizeof(Int16) * VOICE_FRAME);
            printk("\n===============  VOIF_CB_Fxn  before=============== index : %d, isMax : %d\n", index, isMax);
        }
    }
#endif
...
#ifdef ENABLE_DIAMOND_SOLUTION    
    if ( isCall16K == 0 &&  sgCallbackData.DiamondVoice_Mode != 0x00 ) /*8k and when enabled Diamond solution.*/
#endif        
    {            
        memcpy(sgCallbackData.pUldata, ulData, sizeof(Int16) * VOICE_FRAME);
        memcpy(sgCallbackData.pDldata, dlData, sizeof(Int16) * VOICE_FRAME);

#ifdef ENABLE_DIAMOND_SOLUTION    
        sgCallbackData.pDiamondVoiceExe();
#endif
#ifdef ENABLE_NXPEX_SOLUTION 
        sgCallbackData.pLVVEFS_Process(sgCallbackData.pUldata, sgCallbackData.pDldata, sampleCount, isCall16K);
#endif

        memcpy(dlData, sgCallbackData.pDlOutput_buf, sizeof(Int16) * VOICE_FRAME);
        memcpy(ulData, sgCallbackData.pUldata, sizeof(Int16) * VOICE_FRAME);
    ...
    if (sgInjectData.flag ==1 && index<MAX_INJECT )
    {
        if(isMax == 0)
        {
            memcpy(tx_out_array[index], ulData, sizeof(Int16) * VOICE_FRAME);
            memcpy(rx_out_array[index], dlData, sizeof(Int16) * VOICE_FRAME);
...
void VoIF_init(AudioMode_t mode, VoIF_HeadsetType_t hstype)
{
    ...
    sgCallbackData.pDlOutput_buf = (Int16 *)vmalloc(sizeof(Int16) * VOICE_FRAME);
    sgCallbackData.pUlOutput_buf = (Int16 *)vmalloc(sizeof(Int16) * VOICE_FRAME);
    
    sgCallbackData.pDldata = (Int16 *)vmalloc(sizeof(Int16) * VOICE_FRAME);
    sgCallbackData.pUldata = (Int16 *)vmalloc(sizeof(Int16) * VOICE_FRAME);
...
#ifdef ENABLE_DIAMOND_SOLUTION       
    sgCallbackData.DiamondVoice_Mode = sgCallbackData.pDiamondVoice_Config(mode, sgCallbackData.pDldata, sgCallbackData.pUldata, sgCallbackData.pDlOutput_buf);
    printk("\n===============  VoIF_init  =============== AudioMode_t : %d, Mode : %d\n", mode, sgCallbackData.DiamondVoice_Mode);
#endif
...
void VoIF_Deinit()
{
    ...
    vfree(sgCallbackData.pDlOutput_buf);
    vfree(sgCallbackData.pUlOutput_buf);
    ...
    if(!sgCallbackData.isInit)
{
    ...
    printk("\n===============  VoIF_modeChange Mode : %d deviceID : %d=============== \n", mode, deviceID);
#ifdef ENABLE_DIAMOND_SOLUTION
    sgCallbackData.DiamondVoice_Mode = sgCallbackData.pDiamondVoice_Config(mode, sgCallbackData.pDldata, sgCallbackData.pUldata, sgCallbackData.pDlOutput_buf);
#endif    
#ifdef ENABLE_NXPEX_SOLUTION
...
void VoIF_volumeSetting(int vol_index)
{
    ...
    printk("\n===============  VoIF_volumeSetting  =============== callType : %d mode : %d vol : %d, deviceID = %d\n", voifCallType, sgCallbackData.callMode, vol_index, deviceID);
#ifdef ENABLE_NXPEX_SOLUTION
...
void VoIF_setCallType(VoIF_CallType_t callType)
{
    ...
    printk("\n===============  VoIF_setCallType  =============== callType : %d mode : %d, deviceID : %d\n", voifCallType, sgCallbackData.callMode, deviceID);
#ifdef ENABLE_NXPEX_SOLUTION
    sgCallbackData.pLVVEFS_Set_Device(deviceID);
#endif
}
