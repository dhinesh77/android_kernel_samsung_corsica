...
void csl_caph_audioh_config(int path_id, void *p)
{
	...
	case AUDDRV_PATH_HEADSET_OUTPUT:
		...
		if (csl_caph_GetMulticastChannel() != AUDIO_CHANNEL_NUM_NONE) {
			aTrace(LOG_AUDIO_CNTLR, "%s, set stereo channel to allow HS_R\n", __func__);
			chal_audio_hspath_set_mono_stereo(handle, FALSE);
		} else {
...
#if defined(CONFIG_D2083_AUDIO)
		if ((!audioh_hs_on) || (csl_caph_GetMulticastChannel() != AUDIO_CHANNEL_NUM_NONE)) {
#else
		if (!audioh_hs_on) {
#endif
			audioh_hs_on = 1;
...
#if defined(CONFIG_D2083_AUDIO)
	if (csl_caph_GetMulticastChannel() != AUDIO_CHANNEL_NUM_NONE) {
		int forced_sample_mode = csl_caph_GetMulticastChannel();

		if (forced_sample_mode == AUDIO_CHANNEL_STEREO_LEFT) {
			chnl_enable = CHAL_AUDIO_CHANNEL_LEFT;
		} else if (forced_sample_mode == AUDIO_CHANNEL_STEREO_RIGHT) {
			chnl_enable = CHAL_AUDIO_CHANNEL_RIGHT;
		} else if (forced_sample_mode == AUDIO_CHANNEL_MONO) {
			chnl_enable = CHAL_AUDIO_CHANNEL_LEFT;
		}

		aTrace(LOG_AUDIO_CNTLR, "%s, replace channel mode with %d\n", __func__, chnl_enable);
	}
#endif
...
#if defined(CONFIG_D2083_AUDIO)
	if (csl_caph_GetMulticastChannel() != AUDIO_CHANNEL_NUM_NONE) {
		chal_audio_hspath_mute(handle, 0);
	}
#endif
}
