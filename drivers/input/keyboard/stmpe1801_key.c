		...
		if (ret) {
			if (key_values.value != 0xffff8f8f8LL) {
				...
				for (j = 0; j < 3; j++) {
					...
					//printk("Keycode(%d) code(%x) row:col(%d:%d) up(%d)\n", stmpe1801->keycode[code], code, row, col, up);
					printk("[STMPE] R:C(%d:%d) flag(%d)\n", row, col, up);			
...
static int __devinit stmpe1801_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef DBG_PRN
	printk("%s pdata addr (%p)\n",__func__, device_data->pdata);
#endif
	...
	printk("%s keymap size (%d)\n",__func__, device_data->pdata->keypad->keymap_data->keymap_size);
#endif
	...
	device_data->input_dev->name = "bcm_keypad_v2"; // STMPE1801_DRV_NAME
