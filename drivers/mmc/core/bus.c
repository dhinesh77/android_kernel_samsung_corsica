...
{
#ifdef CONFIG_DEBUG_FS
	// 20120612, SEC debug.. null pointer exception check
	printk("%s, Below dump stack is not error but just debugging purpose. \n", __func__);
	dump_stack();
	...
	if (mmc_card_present(card)) {
		if (mmc_host_is_spi(card->host)) {
			printk(KERN_INFO "%s, %s: SPI card removed\n",
				__func__, mmc_hostname(card->host));
		} else {
			printk(KERN_INFO "%s, %s: card %04x removed\n",
