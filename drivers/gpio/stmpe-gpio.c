...
static int stmpe_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	printk("---in stmpe_gpio_get---reg =%d, offset=%d, mask=%d, \n", reg, offset,  mask);
...
static void stmpe_gpio_set(struct gpio_chip *chip, unsigned offset, int val)
{
	...
	printk("---in stmpe_gpio_set---reg =%d, offset=%d, mask=%d, val=%d, which=%d\n", reg, offset,  mask, val,which);
					 ...
					 unsigned offset, int val)
{
	...
	printk("---in stmpe_gpio_direction_output---reg =%d, offset=%d, mask=%d, val=%d\n", reg, offset,  mask, val);
					...
					unsigned offset)
{
	...
	printk("---in stmpe_gpio_direction_input---reg =%d, offset=%d, mask=%d, val=%d\n", reg, offset, mask);
...
static int __devinit stmpe_gpio_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "---In the stmpe_gpio_probe() \n");
	printk("---In the stmpe_gpio_probe() \n");
	...
	printk("---stmpe_gpio->chip.base = %d \n", stmpe_gpio->chip.base); 

	stmpe_gpio->irq_base = stmpe->irq_base + STMPE_INT_GPIO(0);

	printk( "before stmpe_enable() \n" );
	ret = stmpe_enable(stmpe, STMPE_BLOCK_GPIO);
	if (ret)
		goto out_free;

	printk( "before stmpe_gpio_irq_init() \n" );
	...
	printk( "---before gpiochip_add(), the chip->base=%d \n", (&stmpe_gpio->chip)->base);
