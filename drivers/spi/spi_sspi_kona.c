...
static int spi_kona_dma_xfer(struct spi_kona_data *spi_kona)
{
	...
	if ((wait_for_completion_interruptible_timeout
		...
		pr_err(" %s SPI Tx DMA Transfer timed out/interrupted\n",
		       __func__);
		pr_err(" %s SPI Tx DMA Transfer timed out/interrupted\n",__func__);
		pr_info("interrupt_status - 0x%x\n", readl(spi_kona->base+0x10));        
		pr_info("  detail int status  - 0x%x\n", readl(spi_kona->base+0x18));        
		pr_info("  scheduler control  - 0x%x\n", readl(spi_kona->base+0x20));        
		pr_info("  STATE_MACHINE_CONTROL  - 0x%x\n", readl(spi_kona->base+0x28));        
		pr_info("  FIFOTX_0_CONTROL  - 0x%x\n", readl(spi_kona->base+0x40));        
		pr_info("  FIFOTX_0_THRESHOLD  - 0x%x\n", readl(spi_kona->base+0x44));
		pr_info("  TASK0_DESC_MSB - 0x%x\n", readl(spi_kona->base+0x100));
		pr_info("  TASK0_DESC_LSB - 0x%x\n", readl(spi_kona->base+0x104));
		pr_info("  STATE_MACHINE_TIMEOUT - 0x%x\n", readl(spi_kona->base+0x2c));
		pr_info("  DMA_TX0_CONTROL - 0x%x\n", readl(spi_kona->base+0x84));
	}
	...
	/* Wait for RX DMA completion */
		...
		pr_info("  scheduler control  - 0x%x\n", readl(spi_kona->base+0x20));        
		pr_info("  STATE_MACHINE_CONTROL  - 0x%x\n", readl(spi_kona->base+0x28));        
		pr_info("  FIFORX_0_CONTROL  - 0x%x\n", readl(spi_kona->base+0x48));        
		pr_info("  FIFORX_0_THRESHOLD  - 0x%x\n", readl(spi_kona->base+0x4C));
		pr_info("  TASK0_DESC_MSB - 0x%x\n", readl(spi_kona->base+0x100));
		pr_info("  TASK0_DESC_LSB - 0x%x\n", readl(spi_kona->base+0x104));
		pr_info("  STATE_MACHINE_TIMEOUT - 0x%x\n", readl(spi_kona->base+0x2c));
		pr_info("  DMA_RX0_CONTROL - 0x%x\n", readl(spi_kona->base+0x80));
	} else
		ret = spi_kona->count;
