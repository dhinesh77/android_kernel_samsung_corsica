...
/*		ret = regulator_is_enabled(keyled_regulator);
		KLDBG("[KEYLED] -->regulator_is_enabled ret = %d \n", ret);*/
		ret = regulator_set_voltage(keyled_regulator,3300000,3300000);
		KLDBG("[KEYLED] -->regulator_set_voltage ret = %d \n", ret);       
		ret = regulator_enable(keyled_regulator);
		KLDBG("[KEYLED] -->regulator_enable ret = %d \n", ret);       
...
/*		ret = regulator_is_enabled(keyled_regulator);
		KLDBG("[KEYLED] -->regulator_is_enabled ret = %d \n", ret);*/
		regulator_disable(keyled_regulator);
		KLDBG("[KEYLED] -->regulator_disable ret = %d \n", ret);
...
int keyled_set_intensity(struct backlight_device *bd)
{
    ...
    if((0 != current_intensity) && (0 != user_intensity))
    return 0;
