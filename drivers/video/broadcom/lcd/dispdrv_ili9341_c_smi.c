...
typedef enum {
       ...
       LCD_PANEL_ID_ONE,
       LCD_PANEL_ID_TWO,
       LCD_PANEL_ID_THREE,   // Cori-Plus Tianma
       LCD_PANEL_ID_FOUR,    // Cori-Plus CPT
...
static Int32 DispSmi_GetID(DISPDRV_HANDLE_T     dispH,UInt32 reg )
{
	   ...
	   UInt32 id1, id2, id3;
		 ...
		 }else if((id1 == 0x61) && (id2 == 0xA4) && (id3 == 0xD0)){
			return LCD_PANEL_ID_THREE;
		 }else if ((id1 == 0x61) && (id2 == 0x44) && (id3 == 0xD0)){
			return LCD_PANEL_ID_FOUR;
         }else{
            return LCD_PANEL_ID_TWO;
         }
	...
	case CTRL_PWR_ON:
		...
		case STATE_PWR_OFF:
			     ...
			     __FUNCTION__ );
					   ...
					   }else if(LCD_PANEL_ID_THREE== panelID){

						   DispSmi_ExecCmndList ( dispH, &ili9341_panel03_Init[0]);
   
					   }else if(LCD_PANEL_ID_FOUR == panelID){

						   DispSmi_ExecCmndList ( dispH, &ili9341_panel04_Init[0]);
                       }else{
...
//*****************************************************************************
//
// Function Name:  DispSmi_panelID
//
// Description:    return LCD Information for AT+HWINDICK=1,1 of DFMS
//
//*****************************************************************************
char * DispSmi_panelID(void)
{
	LCD_DBG( LCD_DBG_INIT_ID,"DispSmi_panelID() 0x%x\n",panelID);

	if(panelID == LCD_PANEL_ID_THREE)
		return "INH_61A4D0\n";
	else if(panelID == LCD_PANEL_ID_FOUR)
		return "INH_6144D0\n";
	else
		return "INH_61A4D0\n";
}
EXPORT_SYMBOL(DispSmi_panelID);
