...
#define LCD_PANEL_ID_READ_ERROR -1

static int panelID = LCD_PANEL_ID_NOT_SET;
...
Int32 DISPDRV_Init(struct dispdrv_init_parms *parms, DISPDRV_HANDLE_T *handle)
{
	...
	if (pPanel->drvState == DRV_STATE_OFF) {
		...
		if ((u8)parms->w1.bits.api_rev != RHEA_LCD_BOOT_API_REV) {
			LCD_DBG(LCD_DBG_ERR_ID, "[DISPDRV] %s: Boot Init API Rev Mismatch(%d vs %d)\n",
				__func__, parms->w1.bits.api_rev, RHEA_LCD_BOOT_API_REV);
			return (-1);
		}
...
Int32 DISPDRV_PowerControl(DISPDRV_HANDLE_T drvH, DISPLAY_POWER_STATE_T state)
{
	...
	case CTRL_PWR_ON:
		...
		{
               		panelID = DISPDRV_ReadID(drvH,0x04);
			printk("[LCD] Panel ID=%f\n", __func__,panelID);
		}
               isFirstboot = 0;
           ...
           else
           {
				...
				if(LCD_PANEL_ID_ONE_L3 == panelID){
					printk("[LCD] Panel %d\n", LCD_PANEL_ID_ONE_L3);
					
					DISPDRV_ExecCmndList(drvH, &sleep_out_seq_DCS_Type_1[0], DCS_TYPE);
					DISPDRV_SetMaxRxSize( drvH );
					DISPDRV_ExecCmndList(drvH, &power_setting_seq_GEN_Type_1[0], GEN_TYPE);
					DISPDRV_SetMaxRxSize( drvH );
					DISPDRV_ExecCmndList(drvH, &writemem_seq_DCS_Type_1[0], DCS_TYPE);
									
               }else if(LCD_PANEL_ID_TWO_L4 == panelID){
					printk("[LCD] Panel %d\n", LCD_PANEL_ID_TWO_L4);
					
					DISPDRV_ExecCmndList(drvH, &sleep_out_seq_DCS_Type_2[0], DCS_TYPE);
					DISPDRV_SetMaxRxSize( drvH );
					DISPDRV_ExecCmndList(drvH, &power_setting_seq_GEN_Type_2[0], GEN_TYPE);
					DISPDRV_SetMaxRxSize( drvH );
					DISPDRV_ExecCmndList(drvH, &writemem_seq_DCS_Type_2[0], DCS_TYPE);
	...
	case CTRL_PWR_OFF:
		if (pPanel->pwrState != STATE_PWR_OFF) {
			...
			if(LCD_PANEL_ID_ONE_L3 == panelID){
				DISPDRV_ExecCmndList(drvH, &power_off_seq_DCS_Type_1[0], DCS_TYPE);  
	        }else if(LCD_PANEL_ID_TWO_L4 == panelID){
				DISPDRV_ExecCmndList(drvH, &power_off_seq_DCS_Type_2[0], DCS_TYPE);  
			}
			else{
	...
	case CTRL_SLEEP_OUT:
		...
		case STATE_SLEEP:
			if(LCD_PANEL_ID_ONE_L3 == panelID){
				DISPDRV_ExecCmndList(drvH, &power_on_seq_DCS_Type_1[0], DCS_TYPE); 
			}
	        else if(LCD_PANEL_ID_TWO_L4 == panelID){
				DISPDRV_ExecCmndList(drvH, &power_on_seq_DCS_Type_2[0], DCS_TYPE); 
			}
			else
			{
