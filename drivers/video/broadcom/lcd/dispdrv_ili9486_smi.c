...
//   { 0,  15, 10, 15 , },    //  CSL_SMI_TIMIMG_T  wrTiming_m;     //  1-4-3-4 => BB 36ns
 // Original   { 0, 8, 4, 8, },
 { 10, 12, 10, 30 , }, //@HW, this value needs to be tuned
...
switch (state) {
	case CTRL_PWR_ON:
		...
		switch (pPanel->pwrState) {
                case STATE_SLEEP:
			DispSmi_ExecCmndList ( dispH, &power_off_seq_ili9486_BOE[0]);
			OSTASK_Sleep( TICKS_IN_MILLISECONDS(120) );
			pPanel->pwrState = STATE_SCREEN_OFF;
		...
		case STATE_SCREEN_ON:
			DispSmi_ExecCmndList ( dispH, &power_off_seq_ili9486_BOE[0]);
			OSTASK_Sleep( TICKS_IN_MILLISECONDS(120) );
			gpio_request(pPanel->rst, "LCD_RST1");
			gpio_direction_output(pPanel->rst, 0);
			gpio_set_value_cansleep(pPanel->rst, 1);
			msleep(10);
